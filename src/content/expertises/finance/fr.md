---
layout: layouts/page.njk
permalink: /fr/expertises/finance/
title: "Finance"
lead: "Une double expertise IT et métier sur toute la chaîne de valeur financière, du front au back office."
description: "Finance — expertise Meritis."
parent:
  label: "Expertises"
  url: "/fr/expertises/"
---


## Notre expertise IT et Métier de la Finance de marchés

![paris la défense ](/assets/images/expertises/finance/inline-1.webp)

La **technologie** est aujourd’hui **omniprésente dans les systèmes d’informations bancaires**. La digitalisation des départements bancaires est devenue en une décennie un élément pivot sur lequel les grands acteurs de la finance se focalisent.

Pour répondre aux nouvelles exigences de leur clientèle ainsi qu’aux contraintes réglementaires de plus en plus nombreuses, les grandes institutions financières n’ont pas d’autres alternatives que d’investir massivement dans leur infrastructure IT.

Meritis met à disposition de ses clients ses **équipes d’experts fonctionnelles et techniques** pour répondre à ces défis.

## Les défis en Finance de marchés

Meritis intervient sur toute la chaîne de valeur Front to Back Office en ingénierie métier (Risque /Quantitatif) jusqu’au développement des applications destinées aux équipes métiers (traders, sales , middle etc..).

Les enjeux réglementaires et environnementaux imposent une évolution constante des pratiques de Banque d’investissement, des gestionnaires d’actifs et assureurs.

Meritis met à disposition de ses clients Banques d’investissement, Asset Managers, Dépositaires, Bourses, ses équipes d’experts fonctionnels et techniques pour répondre à ces défis.

## L’offre Meritis Finance

### Assistances Techniques Unitaires (ATU) ou Groupées (ATG)

Que vous ayez besoin d’une ou plusieurs compétences particulières à court terme ou à plus ou moins long terme, Meritis se charge de mettre à votre disposition nos meilleurs talents.

**Meritis est reconnu pour la qualité du suivi de ses missions et l’expertise de ses consultants.**

### Engagement de résultat

Nos compétences Techniques et Métier nous permettent de couvrir l’ensemble de votre projet de transformation IT, du cadrage et de l’analyse, en passant par la mise en place, jusqu’à l’accompagnement métier sur des projets au forfait, des centres de compétences ou des centres de service.

**Nos experts interviennent chez nos clients pour les aider à identifier les points bloquants.**

## Nos profils d’experts

<div class="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Ingénierie métier</p>
<p class="text-brand-charcoal">Front Office, It Quant, Commando, Risque Management, Règlementaire, Middle Office, Actuariat, Trésorerie, Financement, Pricing, Risques, P&L</p>
</div>
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Product Owner & MOA métier</p>
<p class="text-brand-charcoal">Gestion d’une application ou d’un projet avec une forte expertise métier</p>
</div>
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Support applicatif</p>
<p class="text-brand-charcoal">Support sur les applications Front to back, Applications assurantielle</p>
</div>
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Intégration de progiciel</p>
<p class="text-brand-charcoal">Simcorp, Murex, Calypso, Alto, Orchestrade, Summit, Sophis, Kleva, etc.</p>
<p class="mt-2 text-brand-charcoal">Java, Javascript, C#, C++, Python, Pl/Sql</p>
</div>
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Développement applicatif</p>
<p class="text-brand-charcoal">Gouvernance, Data Engineering, Data science, développeur Big Data, gestion des référentiels</p>
</div>
<div class="rounded-card bg-brand-light p-6">
<p class="text-lg font-semibold text-brand-blue">Métiers Data</p>
<p class="text-brand-charcoal">Audit, coaching, conseil, expertise</p>
</div>
</div>

### Agilité, DevOps, Cloud

**Engagement**

Nous respectons nos engagements quel que soit le type de projet ou le type d’intervention

**Flexibilité**

Nous fournissons des solutions et adaptons nos offres à votre situation et à votre secteur d’activité.

**Multi-Expertise**

Nous construisons l’expertise de nos équipes pour couvrir toute la chaîne de valeur de vos projets

**Excellence**

Nous formons en permanence nos ingénieurs d’affaires aux métiers de nos clients, aux nouvelles méthodologies de gestion de projet et aux évolutions technologiques.

## Vous souhaitez échanger avec un de nos experts ou prendre rendez-vous ?

## Ils en parlent mieux que nous !

_\*c’est la note obtenue lors d’un sondage de satisfactions sur un échantillon de plus de 1 600 clients._

> Meritis a su nous présenter une offre agressive et a réussi à développer son empreinte. Le retour des équipes opérationnelles est très positif, bon suivi des intervenants et très bonne capacité à répondre aux besoins. J’apprécie de travailler avec Meritis. Ils sont à l’écoute, réactifs et apporteurs de solutions.
>
> **Stéphane THIBAULT**
> **Société Générale**

## Pour aller plus loin

[![BforBank fait appel à meritis pour refondre ses applications mobiles](/assets/images/expertises/finance/inline-2.webp)](https://meritis.fr/reference/b-for-bank-refonte-applications-mobiles/)

[![cas client Meritis - blockchain](/assets/images/expertises/finance/inline-3.webp)](https://meritis.fr/reference/creation-dune-plateforme-de-distribution-digitale-pour-une-grande-banque/)

[![](/assets/images/expertises/finance/inline-4.webp)](/fr/contact/)

### Vous souhaitez en découvrir davantage ?

Retrouvez tous nos articles de blog rédigés par nos consultants.

Car chez Meritis, nous sommes convaincus que la transformation numérique et métier passe avant tout par la transmission de la connaissance et des bonnes pratiques.

### Ils nous font confiance !

![](/assets/images/expertises/finance/inline-5.webp)

[Voir nos cas clients](https://meritis.fr/categorie-livre-blanc/cas-clients/)

## Vous souhaitez prendre un rendez-vous, demander un devis ou en savoir plus ?

Contactez nos experts
